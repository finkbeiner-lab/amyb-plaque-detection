...
class ExplainPredictions():
    ...
    def __init__(self, model_input_path, test_input_path, detection_threshold, wandb, save_result):
    ...
    def quantify_plaques(self, df, wandb_result, img_name, result_img, result_masks, boxes, labels, scores):
        ...
        for i in range(len(labels)):
            ...
            if len(boxes)!= 0:
                ...
                # When there is no mask generated by the model
                # if bw_img is None:
                #     bw_img = np.zeros((cropped_img.shape[0], cropped_img.shape[1]), dtype=np.uint8)
                #     pdb.set_trace()
                #     wandb_result.append([img_name, self.wandb.Image(cropped_img), self.wandb.Image(cropped_img), labels[i], 0, 
                #                          (0,0), 0, 0, 0])
