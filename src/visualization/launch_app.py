...
class ExplainPredictions():
    ...
    def __init__(self, model, model_input_path, detection_threshold, save_result, ablation_cam, save_thresholds):
        self.model = model
    ...
    def get_brown_pixel_cnt(self, img):
        ...
        if len(x) != 0:
            if self.save_thresholds:
                ...
                final_save_path = "test_count_threhold.png"
    ...
    def quantify_plaques(self, df, result_img, result_masks, boxes, labels, scores, total_brown_pixels):
        ...
        for i in range(len(labels)):
            ...
            if len(boxes)!= 0:
                ...
                for props in regions:
                    ...
                    data_record = pd.DataFrame.from_records([{ 'label': labels[i] , 'confidence': scores[i],
    ...
    def generate_results(self, test_image):
        ...
        pdb.set_trace()
        if len(masks) != 0 and len(boxes) !=0 and np.count_nonzero(masks) != 0:
                
            result_img, result_masks = self.draw_segmentation_map(image, masks, boxes, labels)

            total_brown_pixels+= self.get_brown_pixel_cnt(image)

            df = self.quantify_plaques(df, wandb_result, result_img, result_masks, boxes, labels, scores, total_brown_pixels)

            # if self.save_result:
            #     mask_img_name = "test_masks.png"
            ...
            #     # Plot detections
            #     detection_img_name = "detection.png"
...
def run(test_image):
    ...
    explain = ExplainPredictions(model, model_input_path = model_input_path, detection_threshold=0.75, 
                                 save_result=True, ablation_cam=True, save_thresholds=False)
    df = explain.generate_results(test_image)
...
description='''
    [Vivek Gopal Ramaswamy](https://www.biorxiv.org/content/10.1101/2023.05.19.541376v1)
<br />
'''
...
with gr.Blocks() as demo:
    gr.HTML(value="""<h1 style="font-weight: 900; margin-bottom: 7px; margin-top: 5px;">
            A Scalable High Throughput Fully Automated Pipeline for the Quantification of Amyloid Pathology in Alzheimer’s 
            Disease using Deep Learning Algorithms </h1><br>
        """)
    gr.Markdown(description)
    with gr.Accordion(label='How to use (step by step):', open=False):
        gr.Markdown('''
            *First, let's edit your image:*        
            + Upload an image of yours which you want to protect (or select from the examples below)
            + Press submit to get your protected photo

            *Testing the Image*
            + upload the pro      
            + Add a prompt to guide the edit (This step is used for testing
    ...
    with gr.Row():  
        with gr.Column():
            test_image = gr.Image(label='Upload the image')
            
            b1 = gr.Button('Submit')
        with gr.Column():
            protected_image = gr.Image(label="Processed Image")
            genimages = gr.Gallery(label="Generated images", show_label=False, 
                    elem_id="gallery")
            outputs = gr.Dataframe(row_count = (2, "dynamic"), col_count=(5, "fixed"), label="Quantifications", headers=["label", "confidence", "cored", "diffuse", "caa"])
            


    b1.click(run, [test_image], [outputs])
    # examples = gr.Examples(examples=examples_list,inputs = [test_image],  outputs=[genimages], cache_examples=False, fn=run)
