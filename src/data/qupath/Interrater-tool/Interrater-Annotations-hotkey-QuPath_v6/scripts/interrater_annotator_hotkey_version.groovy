...
class_names =  ["Cored", "Diffuse", "Coarse-Grained", "Cotton-Wool", "Burned-Out","None"]
key_names = ["Ctrl+C", "Ctrl+F",  "Ctrl+G", "Ctrl+W",  "Ctrl+B","Ctrl+D"]
color_list = [ [0, 255, 255],[255, 153, 102],[150, 79, 239],[160,170,90],[255,0,125],[124,125,255],[0,0,0]]
...
class AnnotatorDialog implements Runnable {
    ...
    String key
    String assignclass
    def AnnotatorDialog(PathObjectHierarchy hierarchy, String key, String assignclass) {
    ...
    PathClass setObjectClass(PathObject object) {
        if (this.key=="Ctrl+D") {
           ...
           if (object.isLocked()==false){
               ...
               annotations = annotations.toSorted{it.getName().toInteger()}
               for (int i = 0; i < annotations.size(); i++) {
                 //if(annotations[i].getName()==null) {
                     annotations[i].setName(i.toString())
                 // }
                }
               resolveHierarchy()
       ...
       def annotations = imageData.getHierarchy().getAnnotationObjects().findAll { it.getROI().getRoiName()!="Rectangle" && it.isLocked()==false}
        for (int i = 0; i < annotations.size(); i++) {
            if(annotations[i].getName()==null) {
                annotations[i].setName((annotations.size()).toString())
            }       
        }
       annotations = annotations.toSorted{it.getName().toInteger()}
...
def build(String keyCombination=null, String assignclass="") {
    def gui = QPEx.getQuPath().getInstance()
    def hier = QPEx.getCurrentHierarchy()
    def app = new AnnotatorDialog(hier, keyCombination, assignclass)
...
for (int i = 0; i < class_names.size(); i++) {
    dictionary[class_names[i]] = key_names[i]
}

install_command_msg = dictionary.toString()


dictionary.each { key, value ->
    println("Key: $key, Value: $value")
   def menu = build("$value","$key")
}
//def menu =  build("CTRL+E","None")
