...
def process_json(path, visualize=False):
    ...
    def makeSubDir(*args):
        full = os.path.join(*args)
        if not os.path.exists(full):
            os.makedirs(full)
        return full


    image_save_dir, mask_save_dir = [makeSubDir(path, sub) for sub in 'images masks'.split()]

    # Mask Folder
    # mask_save_dir = os.path.join(path, "masks")
    # if not os.path.exists(mask_save_dir):
    #     os.makedirs(mask_save_dir)
    #
    # # Image Folder
    # image_save_dir = os.path.join(path, "images")
    # if not os.path.exists(image_save_dir):
    #     os.makedirs(image_save_dir)


    img_names = ['.'.join(os.path.split(img_path)[1].split('.')[:-1]) for img_path in glob.glob(os.path.join(path, "*.mrxs")) if os.path.exists('.'.join(imagePath.split('.')[:-1] + ['json'])])
    ...
    for img_name in img_names:
        # Read the WSI image
        vips_img = Vips.Image.new_from_file(img_name, level=0)
        vips_info = get_vips_info(vips_img)

        json_name = os.path.join(path, img_name + '.json')
        ...
        for ele in tqdm(json_data):
            roiBounds = ele['region_attributes']
        ...
        for ele in tqdm(json_data):
            ...
            for region in ele['region_attributes']:
                ...
                # Get tileX and tileY
                tileX = region['tiles'][0]['tileBounds'][0]
                tileY = region['tiles'][0]['tileBounds'][1]
                ...
                tileX = tileX + int(vinfo['bounds-x'])
                tileY = tileY + int(vinfo['bounds-y'])
...
if __name__ == '__main__':
    # /gladstone/finkbeiner/steve/work/data/npsad_data/vivek/amy-def

    basePath = os.path.join(*('/ gladstone finkbeiner steve work data npsad_data gennadi amy-def'.split()))
    process_json(basePath)

    # wsiPaths = glob.glob(os.path.join(basePath, '*.mrxs'))
